...
app.get('/api/bootstrap',asyncRoute(async(_req,res)=>{const db=await readDb();const {users,paymentTransactions,...safe}=db;res.json(safe)}))
app.post('/api/reset',asyncRoute(async(_req,res)=>{for(const file of await fsp.readdir(uploadDir)){if(file!=='.gitkeep')await fsp.unlink(path.join(uploadDir,file))}const db=await resetDb();broadcast('reset',{at:new Date().toISOString()});const {users,...safe}=db;res.json(safe)}))
...
app.get('/api/billing/subscription',adminOnly,asyncRoute(async(_req,res)=>{
  const db=await readDb();res.json({subscription:db.subscription,plan:findPlan(db.subscription?.planId)||null,recentPayments:(db.paymentTransactions||[]).slice(0,10)})
}))
...
app.post('/api/orders',asyncRoute(async(req,res)=>{
  ...
  const order={id:`OT-${String(seq).padStart(4,'0')}`,customerId:customer.id,vehicleId:vehicle?.id,customer:customer.name,phone:customer.phone,car:vehicle?`${vehicle.brand} ${vehicle.model} ${vehicle.year}`:'Vehículo por completar',plate:vehicle?.plate||req.body.plate||'S/P',serviceArea:req.body.serviceArea||'Mecánica general',affectedAreas:req.body.affectedAreas||[],paintColor:req.body.paintColor||'',tech:req.body.tech||'Por asignar',stage:'Ingreso',progress:8,value:0,delivery:req.body.delivery||'Por programar',color:'#f05a37',mileage:Number(req.body.mileage||vehicle?.mileage||0),fuel:req.body.fuel||'Por registrar',receivedItems:req.body.receivedItems||'Llave',reason:req.body.reason||'',notes:[],evidence:[],quote:{status:'Borrador',taxRate:19,items:[]},createdAt:new Date().toISOString(),history:[{id:id('his'),event:'Orden creada y recepción registrada',at:new Date().toISOString(),author:req.user.name}]}
  await updateDb(d=>{d.orders.unshift(order);d.activity.unshift({id:id('act'),type:'order.created',orderId:order.id,at:new Date().toISOString()});return order});broadcast('order.created',order);res.status(201).json(order)
}))
app.put('/api/orders/:id',asyncRoute(async(req,res)=>{const updated=await updateDb(db=>{const i=db.orders.findIndex(o=>o.id===req.params.id);if(i<0)throw Object.assign(new Error('Orden no encontrada'),{status:404});const before=db.orders[i];db.orders[i]={...before,...req.body,id:before.id};if(req.body.stage&&req.body.stage!==before.stage)db.orders[i].history.push({id:id('his'),event:`Orden avanzó a ${req.body.stage}`,at:new Date().toISOString(),author:req.user.name});return db.orders[i]});broadcast('order.updated',updated);res.json(updated)}))
app.post('/api/orders/:id/notes',asyncRoute(async(req,res)=>{if(!String(req.body.text||'').trim())return res.status(400).json({error:'La nota está vacía'});const note={id:id('not'),author:req.user.name,role:req.user.role,at:new Date().toISOString(),text:String(req.body.text).trim()};await updateDb(db=>{const order=db.orders.find(o=>o.id===req.params.id);if(!order)throw Object.assign(new Error('Orden no encontrada'),{status:404});order.notes.push(note)});broadcast('order.note',{orderId:req.params.id,note});res.status(201).json(note)}))
app.post('/api/orders/:id/evidence',upload.array('files',8),asyncRoute(async(req,res)=>{const items=(req.files||[]).map((f,index)=>({id:id('ev'),url:`/uploads/${f.filename}`,name:f.originalname,type:req.body.type||'Proceso',caption:req.body.caption||'',at:new Date().toISOString(),author:req.user.name,index}));await updateDb(db=>{const order=db.orders.find(o=>o.id===req.params.id);if(!order)throw Object.assign(new Error('Orden no encontrada'),{status:404});order.evidence.push(...items)});broadcast('order.evidence',{orderId:req.params.id,items});res.status(201).json(items)}))
app.put('/api/orders/:id/quote',asyncRoute(async(req,res)=>{const quote=await updateDb(db=>{const order=db.orders.find(o=>o.id===req.params.id);if(!order)throw Object.assign(new Error('Orden no encontrada'),{status:404});order.quote={...order.quote,...req.body};const subtotal=order.quote.items.reduce((sum,x)=>sum+Number(x.qty)*Number(x.price),0);order.value=Math.round(subtotal*(1+Number(order.quote.taxRate||0)/100));return order.quote});broadcast('order.quote',{orderId:req.params.id,quote});res.json(quote)}))

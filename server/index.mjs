...
app.post('/api/orders',asyncRoute(async(req,res)=>{
  ...
  const order={id:`OT-${String(seq).padStart(4,'0')}`,customerId:customer.id,vehicleId:vehicle?.id,customer:customer.name,phone:customer.phone,car:vehicle?`${vehicle.brand} ${vehicle.model} ${vehicle.year}`:'Vehículo por completar',plate:vehicle?.plate||req.body.plate||'S/P',serviceArea:req.body.serviceArea||'Mecánica general',affectedAreas:req.body.affectedAreas||[],paintColor:req.body.paintColor||'',tech:req.body.tech||'Por asignar',stage:'Ingreso',progress:8,value:0,delivery:req.body.delivery||'Por programar',color:'#f05a37',mileage:Number(req.body.mileage||vehicle?.mileage||0),fuel:req.body.fuel||'Por registrar',receivedItems:req.body.receivedItems||'Llave',reason:req.body.reason||'',diagnosis:'',finalDiagnosis:'',arrivalContact,deliveryContact:'',notes:[],evidence:[],quote:{status:'Borrador',taxRate:19,items:[]},createdAt,history:[{id:id('his'),event:'Orden creada y recepción registrada',at:createdAt,author:req.user.name},...(arrivalContact?[{id:id('his'),event:`Vehículo entregado al taller por: ${arrivalContact}`,at:createdAt,author:req.user.name}]:[])]}
...
app.put('/api/orders/:id',asyncRoute(async(req,res)=>{const updated=await updateDb(db=>{
  ...
  db.orders[i]={...before,...req.body,arrivalContact,deliveryContact,id:before.id}
  const at=new Date().toISOString()
  if(req.body.stage&&req.body.stage!==before.stage)db.orders[i].history.push({id:id('his'),event:`Orden avanzó a ${req.body.stage}`,at,author:req.user.name})

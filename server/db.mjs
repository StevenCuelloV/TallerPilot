...
function seed() {
  ...
  return {
    ...
    locations:[{id:'loc_main',name:'Sede principal',address:'Cra. 6 #32-4',city:'Montería',phone:'+57 300 290 2939',isMain:true,createdAt:now()}],
...
export async function ensureDb() {
  ...
  try {
    ...
    if((existing.version||1)<5){
      existing.version=5
      existing.locations=existing.locations?.length?existing.locations:[{id:'loc_main',name:'Sede principal',address:existing.workshop?.address||'',city:existing.workshop?.city||'',phone:existing.workshop?.phone||'',isMain:true,createdAt:now()}]
      existing.notificationReads=existing.notificationReads||{}
      for(const order of existing.orders||[])order.locationId=order.locationId||existing.locations[0].id
      changed=true
    }
